/*
------------------ FIAP --------------------
CP4 Edge - Vinherias com Cloud IoT
Turma: 1ESPZ
Aluno/Autor: Matheus da Costa Barroso (RM 561308)
Professor: Paulo Marcotti (PF2150)
--------------------------------------------
Descrição:
PoC com ESP32 (DOIT DevKit V1) lendo DHT11 (Temperatura/Umidade) e LDR (Luminosidade),
enviando os dados para o ThingSpeak (field1=Temp, field2=Hum, field3=Lum) a cada ~16s.
Evidências: Wokwi + Dashboard ThingSpeak + vídeo time-lapse + foto do hands-on físico.
*/
...
const char* apiKey = "MMPS8HUM2ND32Y20"; // Write API Key
...
void loop() {
  if (WiFi.status() == WL_CONNECTED) {
    ...
    float luminosidade = map(potValue, 0, 4095, 0, 100); // Mapeamento do valor do potenciômetro para simular a velocidade de um carro da Fórmula E (0 a 322 km/h)
